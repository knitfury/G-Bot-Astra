...
const providerSchema = z.object({
  ...
  type: z.enum(types as [ProviderType, ...ProviderType[]]),
  baseUrl: z.url("Enter a valid endpoint URL."),
  key: z.string(),
  model: z.string().min(1, "Enter a model identifier."),
  headers: z.string(),
  ...
  return (
    <form
      ...
      onSubmit={handleSubmit(
        (v) =>
          void action
            .mutateAsync(() => services.providers.save(v, provider?.id))
            .then(onDone)
            .catch(() => {}),
      )}
      noValidate
    >
      ...
      <div className="form-grid">
        <label className="field">
          Connection name
          <input {...register("name")} placeholder="My business AI" />
          {errors.name && (
      ...
      {type === "Generic REST" && (
        <>
          <Badge tone="warning">Advanced · Generic REST</Badge>
          <div className="form-grid">
            <label className="field">
              HTTP method
          ...
          <label className="field">
            ...
            <textarea className="code" rows={4} {...register("body")} />
          </label>
          <div className="form-grid">
            <label className="field">
              Prompt / input mapping
              <input {...register("inputPath")} />
            </label>
            <label className="field">
              Response-content path
              <input {...register("responsePath")} />
            </label>
          </div>
        </>
      )}
      <details>
        <summary className="text-link tiny">
          Advanced headers and capabilities
        </summary>
        ...
        <Notice>
          <div>
            ...
            {type === "Generic REST" && (
              <pre className="code">
                {
                  '{"content":"Test response received","tools":[],"simulated":true}'
                }
              </pre>
            )}
          </div>
      ...
      <div className="form-actions">
        <Button
          ...
          onClick={handleSubmit((v) => void test(v).catch(() => {}))}
        >
          <Flask size={16} />
          {action.isPending ? "Testing / discovering…" : "Test connection"}
        </Button>
...
export function ProviderManager({ embedded = false }: { embedded?: boolean }) {
  const { data } = useSnapshot(),
    action = useAction();
  const [open, setOpen] = useState(false),
    [editing, setEditing] = useState<AIProviderConnection>(),
    [initialType, setInitialType] = useState<ProviderType>("OpenAI-compatible"),
  ...
  return (
    <div className={embedded ? "stack" : "page"}>
      {!embedded && (
        <PageHeading
          eyebrow="YOUR AI, YOUR WAY"
          title="AI providers"
          description="Connect the intelligence that works for you. Your apps stay connected when you switch models."
        >
          <Button
            variant="default"
            onClick={() => {
      ...
      {data.providers.length === 0 ? (
        <Empty
          ...
          description="Choose a provider above. Add more than one and switch models whenever you need."
        />
      ) : (
          ...
          {data.providers.map((p) => (
            <article className="panel provider-card" key={p.id}>
              ...
              <div className="provider-details">
                ...
                <span>
                  {p.models.length} models · {p.model}
                </span>
                <span>
                  Last tested{" "}
                  {p.lastTest
                    ? new Date(p.lastTest).toLocaleDateString()
                    : "Never"}
                </span>
              </div>
              {tested === p.id && (
                <Notice>Connection test passed. Models are available.</Notice>
              )}
              <div className="row">
                <Button
                  size="sm"
                ...
                <Button
                  ...
                  onClick={() =>
                    void action
                      .mutateAsync(() => services.providers.testSaved(p.id))
                      .then(() => setTested(p.id))
                      .catch(() => {})
                  }
                >
      ...
      <Dialog
        ...
        title={editing ? "Edit AI provider" : "Connect an AI provider"}
        description="Configure a provider without changing your business app connections."
        wide
      >
        {open && (
          <ProviderForm
      ...
      >
        <div className="form-actions">
          ...
          <Button
            ...
            onClick={() =>
              void action
                .mutateAsync(() => services.providers.remove(remove!))

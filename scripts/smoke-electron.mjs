...
try{
 ...
 const demoWindow=app.waitForEvent('window');await page.getByRole('button',{name:'Explore Demo Mode',exact:true}).click();const demo=await demoWindow;await demo.locator('html[data-app-ready="true"]').waitFor({timeout:60000});await demo.getByRole('button',{name:'Explore the demo',exact:true}).click();await demo.getByRole('heading',{name:'What can we get done?'}).waitFor();
 ...
 for(const [index,page] of app.windows().entries()){
  await page.screenshot({path:`test-results-electron/failure-${packaged?'packaged-':''}${index}.png`}).catch(()=>{});
  console.error('Native failure page',index,page.url(),await page.locator('body').innerText().catch(()=>''));
 }
 throw error;

...
export function retain(
  ...
  next.conversations = next.conversations.filter(
    (c) => Date.parse(c.updatedAt) >= cutoff,
  );
  const ids = new Set(next.conversations.map((c) => c.id));
  next.approvals = next.approvals.filter((a) => ids.has(a.conversationId));
...
export function exportHistory(db: Database, format: "json" | "markdown") {
  ...
  return format === "json"
    ? JSON.stringify(
        { format: "g-bot-history", version: 1, conversations },
        null,
        2,
      )
    : conversations
...
export function portableData(db: Database): Database {
  ...
  copy.providers = copy.providers.map((p) => ({
    ...p,
    headers: "{}",
    maskedCredential: "Reconnect after restore",
    status: "disconnected",
  }));

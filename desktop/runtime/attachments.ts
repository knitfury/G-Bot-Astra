...
export class Attachments {
  constructor(private workerPath = join(__dirname, "extract-worker.cjs")) {}
  ...
  async ingestFile(
    ...
    if (bytes.length > 50 * 1024 * 1024)
      throw new DomainError("CAPABILITY", "Maximum file size is 50 MB.");
    if (/\.(png|jpe?g|webp)$/i.test(name)) {
      ...
      if (!png && !jpg && !webp)
        throw new DomainError(
          "CAPABILITY",
          "Image content does not match a supported format.",
        );
    ...
    if (!/\.(pdf|docx|xlsx|txt|md|csv)$/i.test(name))
      return {
        id: randomUUID(),
        name,
        kind: "file",
        ...
        size: bytes.length,
        status: "unsupported",
        error: "Use PDF, DOCX, TXT, MD, CSV, XLSX, PNG, JPEG or WebP.",
      };
    const text = await new Promise<string>((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        ...
        reject(
          new DomainError(
            "CAPABILITY",
            "Extraction timed out. Try a smaller document.",
          ),
        );
      }, 30000);
      worker.once(
        ...
        (result: { ok: boolean; text?: string; error?: string }) => {
          clearTimeout(timer);
          void worker.terminate();
          if (result.ok && result.text) resolve(result.text);
          else
            reject(
              new DomainError(
                "CAPABILITY",
      ...
      worker.once("error", () => {
        ...
        reject(
          new DomainError(
            "CAPABILITY",
            "Extraction failed safely. Try another document.",
          ),
        );
      });
      worker.once("exit", (code) => {
        ...
        if (code !== 0)
          reject(
            new DomainError(
              "CAPABILITY",
              "Extraction stopped. Try a smaller document.",
            ),
          );
      });
    });
    ...
    const selected = chunks.slice(0, 12).join("\n");
    this.content.set(id, {
      text:
        selected +
        (chunks.length > 12
